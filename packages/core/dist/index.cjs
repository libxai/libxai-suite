...
var BoardStore = class extends Store {
   ...
   */
  setBoard(boardData) {
    const board = exports.Board.fromData(boardData);
    this.setState((state) => ({
      ...state,
      board
    }));
  ...
  addColumn(columnData) {
    const column = new exports.Column(columnData);
    this.setState((state) => {
   ...
   */
  addCard(cardData) {
    const card = new exports.Card(cardData);
    this.setState((state) => {
  ...
  constructor(options) {
    ...
    if (options.initialData) {
      if (options.initialData.board) {
        initialState.board = new exports.Board(options.initialData.board);
      }
      if (options.initialData.columns) {
        options.initialData.columns.forEach((col) => {
          initialState.columns.set(col.id, new exports.Column(col));
        });
      }
      if (options.initialData.cards) {
        options.initialData.cards.forEach((card) => {
          initialState.cards.set(card.id, new exports.Card(card));
        });
      }
    }
...
exports.AsakaaRuntime = AsakaaRuntime;
exports.BaseSerializer = BaseSerializer;
exports.BaseViewAdapter = BaseViewAdapter;
exports.BinarySerializer = BinarySerializer;
exports.BoardController = BoardController;
exports.BoardStore = BoardStore;
exports.DependencyEngine = DependencyEngine2;
...
exports.createBinarySerializer = createBinarySerializer;
